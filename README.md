...
| `voidcore` | abyss · frost · ember · verdant · violet | all | no | A face-on filament disc that **reacts to live signal** |
...
As `pulse` rises the disc takes a rotational kick, its inner filament tiers
fade up and the light lifts off the unlit arc. Given a `focus`, the disc leans
toward it and the flow brightens where it is being stirred, easing back over
~600ms.
